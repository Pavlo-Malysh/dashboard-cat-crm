...
- **`index.html`** — головний дашборд (Overview): welcome, co-workers, tasks, reports.
- **`companies-page.html`** — сторінка списку компаній-партнерів (картки з назвою, локацією, іконкою та кнопкою Discover).

## Основний функціонал

- **Бічне меню (Sidebar)** — навігація з іконками, адаптивне відкриття/закриття на мобільних пристроях (з блокуванням скролу сторінки). Активний пункт меню (`current`) визначається автоматично на клієнті: скрипт порівнює `location.pathname` з `href` кожного посилання і сам проставляє/знімає клас — окремо для кожної сторінки, без ручного дублювання партіалу.
...
- **Companies** — сітка карток компаній-партнерів (grid, адаптивна кількість колонок: 1 на мобільних, 2 на планшетах, 3 на десктопі).
...
│   ├── js/
│   │   └── main.js        # Точка входу: імпорт стилів, sidebar-меню, active-link логіка
│   ├── partials/           # Спільні фрагменти розмітки (sidebar.html, header.html),
│   │                        # підключаються через <load src="..."/> (vite-plugin-html-inject)
│   └── scss/               # Стилі проєкту (детально нижче)
├── dist/                  # Згенерована продакшн-збірка (npm run build)
├── index.html              # Головний дашборд (Overview)
├── companies-page.html      # Сторінка списку компаній
...
- **Партіали та вкладеність папок.** Кожен компонент — окремий партіал (`_badges.scss`, `_menu.scss`, `_reports.scss`, `_tasks.scss`, `_welcome.scss`, `_companies-list.scss` тощо), а складніші компоненти виділені у вкладені підпапки з власним `index.scss`, що імпортує внутрішні частини:
  ...
  - `components/_companies-list.scss` — сітка карток на сторінці Companies: власний `display: grid` з кількістю колонок за брейкпоінтами (1 → 2 → 3), без залежності від flex-компонента `.board`.
